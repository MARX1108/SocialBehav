...
bootstrap_glm <- function(df, T, plot=TRUE, filesname)
{
  ...
  finaldf <- twoclasssample(df, n=2000)
  ...
  for (t in 1:T){
    ...
    glm.fit <- glm(y ~ ., data = train, family = binomial)
    #fitted.value <- predict(glm.fit,newdata = train,type="response")

  
    cm <- confusionMatrix(table(as.numeric(glm.fit$fitted.value>0.5),train$y))
...
```{r}
cm_tb<-bootstrap_glm(df, T=GlobalT, filesname = './Aggression-Attributes-brain.csv')
write.csv(cm_tb,"./Aggression-Results-brain.csv", row.names = FALSE)
```
...
```{r}
cm_tb<-bootstrap_glm(df, T=GlobalT, filesname = './Aggression-Attributes-brain+EPF.csv')
write.csv(cm_tb,"./Aggression-Results-brain+EPF.csv", row.names = FALSE)
```
