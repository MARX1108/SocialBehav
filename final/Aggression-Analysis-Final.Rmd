...
aggression_preprocessing <- function(df)
{
  ...
  df<- df[(df$aggressive_sumscore  <= (-2.0205)) | (df$aggressive_sumscore  >= (0.2168869)), ]
  df$y <- ifelse(df$aggressive_sumscore <= (-2.0205), 0, 1)
  return(df)
}
...
```{r}
cm_tb<-bootstrap_glm(df, T=GlobalT, filesname = './Aggression-A-EPF.csv', filesname2 = './T-Aggression-A-EPF.csv', filesname3 = './Aggression-A-Beta-EPF.csv')
write.csv(cm_tb,"./Aggression-R-EPF.csv", row.names = FALSE)
```
...
df <- aggression_preprocessing(df)
df <- subset(df, select=-c(aggressive_sumscore, prosocial_child, prosocial_parent, subjectkey))
```

```{r}
cm_tb<-bootstrap_glm(df, T=GlobalT, filesname = './Aggression-A-brain.csv',filesname2 = './T-Aggression-A-brain.csv', filesname3 = './Aggression-A-Beta-brain.csv')
write.csv(cm_tb,"./Aggression-R-brain.csv", row.names = FALSE)
```
...
```{r}
cm_tb<-bootstrap_glm(df, T=GlobalT, filesname = './Aggression-A-brain+EPF.csv', filesname2 = './T-Aggression-R-brain+EPF.csv', filesname3 = './Aggression-A-Beta-brain+EPF.csv')
write.csv(cm_tb,"./Aggression-R-brain+EPF.csv", row.names = FALSE)
```
