...
label = 1; if aggressive_sumscore >=  66% (-0.686183125)
label = -1; if aggressive_sumscore minimum< x < 66% (-0.686183125)
Model: Decision tree
"""
...
def main():
	...
	agg_data.to_csv('result_66_tri.csv')
	# ..print('agg_data shape: ', agg_data.shape)
	# ..print(agg_data['aggressive_sumscore'].value_counts())
	...
	print('f1 score: ', f1_score(validation_set[3], y_pred, average='macro'))
	print('recall: ', recall_score(validation_set[3], y_pred, average='macro'))
	print('precision: ', precision_score(validation_set[3], y_pred, average='macro'))
	print('kappa: ', cohen_kappa_score(validation_set[3], y_pred, labels=[0, 1, -1]))
...
def data_preprocess():
	...
	for i in range(len(data.index)):
		...
		if old_value >= q66 and old_value != 0:
			data['aggressive_sumscore'] = data['aggressive_sumscore'].replace([old_value], 1)
		elif minimum < old_value < q66 and old_value != 0:
			data['aggressive_sumscore'] = data['aggressive_sumscore'].replace([old_value], -1)
